...
//----------------------------------------------------------------------------------
//string text01 = "Lorem ipsum dolor sit amet, \xE7\x8C\xBF\xE3\x82\x82\xE6\x9C\xA8\xE3\x81\x8B\xE3\x82\x89\xE8\x90\xBD\xE3\x81\xA1\xE3\x82\x8B consectetur adipiscing elit"; // including some hiragana/kanji
string text01 = "Lorem ipsum dolor sit amet,consectetur adipiscing elit"; // including some hiragana/kanji
string text02 = "Here's another, much bigger textboxBounds." ~ "\xf4\xa1\xa1\xff" ~ " TIP: try COPY/PASTE ;)"; // including some invalid UTF8

int textboxActive = 0; // Keeps traks of the active textboxBounds

Rectangle[] textboxBounds = [Rectangle(20, 60, 160, 25), Rectangle(220, 60, 300, 25), Rectangle(565, 60, 95, 25),
...
bool ColorButton(Rectangle bounds, Color color) {
   Rectangle rbody = Rectangle(bounds.x + 2, bounds.y + 2, bounds.width - 4, bounds.height - 4);
   bool pressed = false;

   // Update control
   Vector2 mouse = GetMousePosition();
   if (IsMouseButtonPressed(MouseButton.MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(mouse, bounds)) {
      pressed = true;
   }

   // Draw control
   DrawTexture(checked, rbody.x, rbody.y, WHITE);
   DrawRectangleRec(rbody, color);
...
void main(string[] args) {
   ...
   while (!WindowShouldClose()) // Detect window close button or ESC key
   {
      ...
      if (textBox01EditMode)
         textboxActive = 0;
      else if (textBox02EditMode)
         textboxActive = 1;
      else if (spinnerEditMode)
         textboxActive = 2;
      else if (valueBoxEditMode)
         textboxActive = 3;

      // Show/Hide the textboxBounds menu
      /*
        if ((textboxActive < 2) && !showMenu && CheckCollisionPointRec(mouse, textboxBounds[textboxActive]) && IsMouseButtonPressed(MouseButton.MOUSE_RIGHT_BUTTON))
        {
            showMenu = true;
      ...
      // Menu hidding logic
      if (showMenu && IsMouseButtonPressed(MouseButton.MOUSE_LEFT_BUTTON) && !CheckCollisionPointRec(mouse, menuRect))
         showMenu = false;
      ...
      if (IsFileDropped()) {
         ...
         if (IsFileExtension(files[0], ".ttf") || IsFileExtension(files[0], ".otf") || IsFileExtension(files[0], ".fnt")) {
      ...
      /+
        ...
        string text = (textboxActive == 1) ? text02 : text01;
        int maxSize = (textboxActive == 1) ? text02.sizeof : text01.sizeof;
...
+/
      ...
      if (GuiTextBoxEx(textboxBounds[0], text01, SIZEOF(text01) - 1, textBox01EditMode))
         textBox01EditMode = !textBox01EditMode;
      if (GuiTextBoxEx(textboxBounds[1], text02, SIZEOF(text02) - 1, textBox02EditMode))
         textBox02EditMode = !textBox02EditMode;

      if (GuiSpinner(textboxBounds[2], NULL, &spinnerValue, INT_MIN, INT_MAX, spinnerEditMode))
         spinnerEditMode = !spinnerEditMode;
      if (GuiValueBox(textboxBounds[3], NULL, &valueBoxValue, INT_MIN, INT_MAX, valueBoxEditMode))
      ...
      if (showMenu) {
         ...
         string[] menuItems = ["#17# Cut", "#16# Copy", "#18# Paste", "#101# SelectAll"];
         ...
         if (active != -1) {
            ...
            switch (active) {
               ...
               case 1:
                  ...
                  break;
               case 2:
                  GuiTextBoxPaste(text, (textboxActive == 1) ? SIZEOF(text02) : SIZEOF(text01));
                  break;
               case 3:
